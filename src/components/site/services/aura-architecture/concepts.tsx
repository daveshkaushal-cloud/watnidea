...
/* ------------------------------------------------------------------ *
 * AuraConcepts — Section 5 of /aura-architecture
 *
 ...
 *
 * Reads from siteContent.CASE_STUDIES — only items with
 * services.includes('aura-architecture'). All verified=false today.
 * NO fake clients, NO fake metrics, NO fake testimonials.
 * ------------------------------------------------------------------ */
...
const AURA_CONCEPTS = CASE_STUDIES.filter(
  (c) => c.services.includes('aura-architecture') && !c.verified,
)

export function AuraConcepts() {
  return (
    <Section surface="white" aria-labelledby="aura-concepts-heading">
      <Container>
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <SectionLabel number="04" accent="#F13D32">
              Concept exploration
            </SectionLabel>
            <Reveal delay={0.08}>
              <EditorialHeading id="aura-concepts-heading" className="mt-5 max-w-[18ch]">
                Concepts on the <Underline>wall</Underline>.
              </EditorialHeading>
            </Reveal>
          </div>
          <Reveal delay={0.16}>
        ...
        <Reveal delay={0.2}>
          <p className="mt-5 max-w-xl text-base text-[#5D5A54]">
            ...
            meantime, these are studio-side explorations of how an
            identity system can behave.
          </p>
        </Reveal>

        {/* Concept tiles */}
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2">
          {AURA_CONCEPTS.map((c, i) => (
            <Reveal key={c.slug} delay={0.24 + i * 0.08}>
              ...
              >
                <div
                  ...
                  style={{ background: '#F13D32', color: '#FFFFFF' }}
                >
                  <span aria-hidden className="wn-halftone-light absolute inset-0 opacity-25" />
