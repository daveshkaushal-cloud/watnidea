...
/**
 * Kinetic Studio — shared helpers (Orange accent system).
 *
 ...
 *
 * Kinetic Studio's service accent is ORANGE (#F97316) per the global color
 * directive. To honor that WITHOUT modifying the shared file (which every
 * other page depends on), this module provides orange-accented equivalents.
 * Kinetic Studio section components import from HERE instead of
 ...
 *   - Deep Red (#E53935) — supporting (brand color, cinematic warmth)
 *   - Warm Cinematic Gold (#FBBF24 / #F59E0B) — highlights
 *   - Premium dark aesthetic (#141414 base)
 */
...
/* ===================================================================
   KINETIC color system — Orange primary + deep red + warm gold supports.
...
export const KINETIC = {
  ...
  neon: '#fdba74', // orange-300 (neon highlight)
  deep: '#c2410c', // orange-700 (deep burnt orange)
  red: '#E53935', // deep red (brand, supporting)
  gold: '#FBBF24', // warm cinematic gold (highlight)
  glow: 'rgba(249,115,22,0.45)',
} as const

/* ===================================================================
   OrangeGradientText — orange→gold gradient text span with optional glow.
   `bg-gradient-to-br from-[#fdba74] via-[#F97316] to-[#c2410c]`
   =================================================================== */
export function OrangeGradientText({
  children,
  className = '',
  glow = true,
}: {
  ...
  return (
    <span
      className={
        ...
        (glow ? 'drop-shadow-[0_0_30px_rgba(249,115,22,0.45)] ' : '') +
        className
      }
    >
      {children}
...
/* ===================================================================
   OrangeEyebrow — `(NN)` orange number + orange rule + label.
   =================================================================== */
export function OrangeEyebrow({
  number,
  label,
}: {
  number: string
  ...
  return (
    ...
    >
      ...
      <span className="h-px w-8 bg-[#F97316]/60" />
...
/* ===================================================================
   OrangeStickyRail — sticky vertical rail (lg+ only) with orange
   progress line driven by useScroll on the supplied sectionRef.
   =================================================================== */
export function OrangeStickyRail({
  label,
  caption,
  sectionRef,
}: {
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="relative h-56 w-px overflow-hidden rounded-full bg-white/10">
            <motion.div
              ...
              className="absolute inset-0 origin-top bg-[#F97316]"
            />
          </div>
...
/* ===================================================================
   OrangeAmbient — layered slow-pulsing orange + deep red + warm gold
   glow blobs. pointer-events-none, absolute inset-0.
   =================================================================== */
export function OrangeAmbient() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      />
      {/* intense orange, top-center, slow pulse */}
      <motion.div
        aria-hidden
        className="absolute left-1/2 top-[8%] h-[68vw] w-[68vw] -translate-x-1/2 rounded-full"
        style={{
          background:
            'radial-gradient(circle, rgba(249,115,22,0.22), rgba(249,115,22,0.06) 40%, rgba(249,115,22,0) 70%)',
          filter: 'blur(50px)',
        }}
        animate={{ opacity: [0.55, 0.95, 0.55], scale: [1, 1.08, 1] }}
        transition={{ duration: 11, repeat: Infinity, ease: 'easeInOut' }}
      />
      {/* secondary deep-red blob, lower-left */}
      <motion.div
        aria-hidden
      ...
      />
      {/* warm gold highlight, upper-right */}
      <motion.div
        aria-hidden
        className="absolute right-[-4%] top-[18%] h-[26vw] w-[26vw] rounded-full"
        style={{
          background:
    ...
    <canvas
      ref={(c) => {
        ...
        const spawn = (initial = false) => ({
          ...
          // orange (25) vs gold (45) vs red (8)
          hue: Math.random() < 0.5 ? 25 : Math.random() < 0.5 ? 45 : 8,
          sat: Math.random() < 0.6 ? 92 : 85,
        })
        ...
        const draw = () => {
          ...
          for (let i = 0; i < embers.length; i++) {
            ...
            grad.addColorStop(0, `hsla(${p.hue}, ${p.sat}%, 62%, ${alpha})`)
            grad.addColorStop(1, `hsla(${p.hue}, ${p.sat}%, 45%, 0)`)
            ctx.fillStyle = grad
            ctx.beginPath()
...
/* ===================================================================
   OrangeEnergySphere — 4-layer cursor-follow orange energy sphere
   (cinematic light source). Concentric pulsing orange + gold glows +
   cursor follow via useMotionValue → useSpring → useTransform + dynamic
   lighting overlay via useMotionTemplate. Accepts a `size` prop for
   reuse at different scales (size = base viewport-relative width, e.g.
   70 → 70vw / 680px max).
   =================================================================== */
export function OrangeEnergySphere({
  size = 70,
}: {
  size?: number
}) {
  ...
  const lightGrad = useMotionTemplate`radial-gradient(circle at ${lightX} ${lightY}, rgba(249,115,22,0.55), rgba(249,115,22,0) 55%)`
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          style={{
            ...
            maxHeight: outerMax,
            background:
              'radial-gradient(circle, rgba(249,115,22,0.45), rgba(249,115,22,0.08) 40%, rgba(249,115,22,0) 70%)',
            filter: 'blur(30px)',
          }}
        ...
        <motion.div
          ...
          style={{
            ...
            maxHeight: midMax,
            background:
              'radial-gradient(circle, rgba(251,191,36,0.55), rgba(249,115,22,0.2) 50%, rgba(249,115,22,0) 75%)',
            filter: 'blur(20px)',
          }}
        ...
        <motion.div
          ...
          style={{
            ...
            maxHeight: coreMax,
            background:
              'radial-gradient(circle, rgba(254,215,170,0.85), rgba(249,115,22,0.4) 60%, rgba(249,115,22,0))',
            filter: 'blur(8px)',
          }}
      ...
      {/* Dynamic cursor-follow orange lighting overlay */}
      <motion.div
...
/* ===================================================================
   OrangeMagneticButton — orange primary / outline secondary variant of
   the hero MagneticButton. Same magnetic-follow + shine + glow
   behavior, but tinted orange so it matches the Kinetic Studio accent.
   =================================================================== */
export function OrangeMagneticButton({
  children,
  className,
  variant = 'primary',
  cursorLabel,
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        isPrimary
          ? 'bg-[#F97316] text-[#141414]'
          : 'border border-white/25 bg-white/5 text-white backdrop-blur-md hover:border-[#F97316]/60',
        className
      )}
    >
      {/* Shine sweep */}
      ...
      {isPrimary && (
        <span
          ...
          style={{
            background:
              'radial-gradient(circle at center, rgba(249,115,22,0.7), rgba(249,115,22,0))',
          }}
        />
      )}
