...
/**
 * Synthetic Cinema — shared helpers (Purple accent system).
 *
 ...
 *
 * Synthetic Cinema's service accent is PURPLE (#8B5CF6) per the global
 * color directive. To honor that WITHOUT modifying the shared file (which
 * every other page depends on), this module provides purple-accented
 * equivalents. Synthetic Cinema section components import from HERE instead
...
/* ===================================================================
   SYNTH color system — Purple primary + electric violet + magenta supports.
...
export const SYNTH = {
  hex: '#8B5CF6',
  rgb: '139,92,246',
  soft: '#a78bfa', // violet-400 (electric violet)
  neon: '#d946ef', // fuchsia-500 (magenta highlight)
  deep: '#6d28d9', // violet-700 (dark purple)
  glow: 'rgba(139,92,246,0.45)',
} as const

/* ===================================================================
   PurpleGradientText — purple gradient text span with optional glow.
   `bg-gradient-to-br from-[#a78bfa] via-[#8B5CF6] to-[#6d28d9]`
   =================================================================== */
  ...
  return (
    <span
      className={
        ...
        (glow ? 'drop-shadow-[0_0_30px_rgba(139,92,246,0.45)] ' : '') +
        className
      }
    >
      {children}
...
/* ===================================================================
   PurpleEyebrow — `(NN)` purple number + purple rule + label.
   =================================================================== */
export function PurpleEyebrow({
  number,
  label,
}: {
  number: string
...
/* ===================================================================
   PurpleStickyRail — sticky vertical rail (lg+ only) with purple
   progress line driven by useScroll on the supplied sectionRef.
   =================================================================== */
export function PurpleStickyRail({
  label,
  caption,
  sectionRef,
}: {
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="relative h-56 w-px overflow-hidden rounded-full bg-white/10">
            <motion.div
              ...
              className="absolute inset-0 origin-top bg-[#8B5CF6]"
            />
          </div>
...
/* ===================================================================
   PurpleAmbient — layered slow-pulsing purple + dim white glow blobs.
...
export function PurpleAmbient() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      />
      {/* intense purple, top-center, slow pulse */}
      <motion.div
        aria-hidden
        className="absolute left-1/2 top-[8%] h-[68vw] w-[68vw] -translate-x-1/2 rounded-full"
        style={{
          background:
            'radial-gradient(circle, rgba(139,92,246,0.22), rgba(139,92,246,0.06) 40%, rgba(139,92,246,0) 70%)',
          filter: 'blur(50px)',
        }}
        animate={{ opacity: [0.55, 0.95, 0.55], scale: [1, 1.08, 1] }}
        transition={{ duration: 11, repeat: Infinity, ease: 'easeInOut' }}
      />
      {/* secondary magenta blob, lower-left */}
      <motion.div
        aria-hidden
    ...
    <canvas
      ref={(c) => {
        ...
        const spawn = (initial = false) => ({
          ...
          // purple (265) vs violet (275) vs magenta (300)
          hue: Math.random() < 0.5 ? 265 : Math.random() < 0.5 ? 275 : 300,
        })
        ...
        const draw = () => {
          ...
          for (let i = 0; i < embers.length; i++) {
            ...
            grad.addColorStop(0, `hsla(${p.hue}, 85%, 65%, ${alpha})`)
...
/* ===================================================================
   PurpleEnergySphere — 4-layer cursor-follow purple energy sphere.
   Concentric pulsing purple glows + cursor follow via
   ...
   =================================================================== */
export function PurpleEnergySphere({
  size = 70,
}: {
  size?: number
}) {
  ...
  const lightGrad = useMotionTemplate`radial-gradient(circle at ${lightX} ${lightY}, rgba(139,92,246,0.55), rgba(139,92,246,0) 55%)`
      ...
      >
        ...
        <motion.div
          ...
          style={{
            ...
            maxHeight: outerMax,
            background:
              'radial-gradient(circle, rgba(139,92,246,0.45), rgba(139,92,246,0.08) 40%, rgba(139,92,246,0) 70%)',
            filter: 'blur(30px)',
          }}
        ...
        <motion.div
          ...
          style={{
            ...
            maxHeight: midMax,
            background:
              'radial-gradient(circle, rgba(167,139,250,0.6), rgba(139,92,246,0.2) 50%, rgba(139,92,246,0) 75%)',
            filter: 'blur(20px)',
          }}
        ...
        <motion.div
          ...
          style={{
            ...
            maxHeight: coreMax,
            background:
              'radial-gradient(circle, rgba(196,181,253,0.85), rgba(139,92,246,0.4) 60%, rgba(139,92,246,0))',
            filter: 'blur(8px)',
          }}
...
/* ===================================================================
   PurpleMagneticButton — purple primary / outline secondary variant of
   the hero MagneticButton. Same magnetic-follow + shine + glow behavior,
   but tinted purple so it matches the Synthetic Cinema accent system.
   =================================================================== */
export function PurpleMagneticButton({
  children,
  className,
  variant = 'primary',
  cursorLabel,
    ...
    <motion.button
      ...
      className={cn(
        ...
        isPrimary
          ...
          : 'border border-white/25 bg-white/5 text-white backdrop-blur-md hover:border-[#8B5CF6]/60',
        className
      )}
    >
      {/* Shine sweep */}
      ...
      {isPrimary && (
        <span
          ...
          style={{
            background:
              'radial-gradient(circle at center, rgba(139,92,246,0.7), rgba(139,92,246,0))',
          }}
        />
      )}
